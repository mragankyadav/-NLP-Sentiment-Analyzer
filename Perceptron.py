...
class Perceptron:
  ...
  def classify(self, words):
    ...
    testVector={}
    for i in words:
      if testVector.get(i)==None:
        testVector[i]=1
      else:
        testVector[i]+=1
    total=0
    for k,v in testVector.iteritems():
      if self.w0.get(k)!=None:
        total+=(self.w0[k]-self.wA[k]/self.steps)*testVector[k]
    total+=self.curr_bias-(self.bias_avg/self.steps)
  ...
  def addExample(self, klass, words):
    ...
    if self.g==0:
      for i in frequency:
        if self.w0.get(i)==None:
          self.w0[i]=0
          self.wA[i]=0
    total = 0
    for word in frequency:
      total += (frequency[word]) * self.w0[word]
    total+=self.curr_bias
    # print total
    if y*total<=0:
      for word in frequency:
        self.w0[word]+=y*frequency[word]
        self.wA[word]+=y*frequency[word]*self.steps
      self.bias_avg+=y*self.steps
      self.curr_bias += y
    self.steps+=1
