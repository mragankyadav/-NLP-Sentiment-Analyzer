...
class NaiveBayes:
  ...
  def classify(self, words):
    ...
    totpos,totneg=self.wordsInClass()
    ppos=0
    pneg=0
    if not self.BOOLEAN_NB:
      ...
      for i in words:
        c=0
        if self.bag["pos"].get(i)!=None:
          c=self.bag["pos"][i]
        pword=(c+1)/(totpos+self.vocab+1)
        ppos+=math.log(pword,10)
      ppos+=math.log(.5,10)

      #for neg class
      for i in words:
        c=0
        if self.bag["neg"].get(i)!=None:
          c=self.bag["neg"][i]
        pword=(c+1)/(totneg+self.vocab+1)
        pneg+=math.log(pword,10)
      pneg+=math.log(.5,10)
    else:
  ...
  def wordsInClass(self):
    poswords=0
    negwords=0
    for k,v in self.bag["pos"].iteritems():
      poswords+=v
    for k,v in self.bag["neg"].iteritems():
      negwords+=v
  ...
  def addExample(self, klass, words):
     ...
     * Returns nothing
    """
    if self.bag.get(klass)==None:
      self.bag[klass]={}
    for i in words:
      if (self.bag[klass]).get(i.lower())==None:
        self.bag[klass][i.lower()]=1
        self.vocab+=1
      else:
        self.bag[klass][i.lower()]+=1
